...
function FilterAuditTrail({startLoading}) {
  ...
  const getActionTypes = () => {
    App.showNotifiction("default", "Fetching Report");
    App.getRequest(urls.getAuditActions).then(result => {
      if (result && result.status) {
        setReportTypes(result.data);
        App.showNotifiction("success", "Report Gotten");
      }
    })
  }
  ...
  const getReport = () => {
    ...
    startLoading(true);
    App.postRequest(urls.filterAudit, {
      SearchKeyword: staffId ? staffId : reportType,
      StartDate: dateSelected[0],
      EndDate:  dateSelected[1],
    }).then(result => {
      if (result && result.status ) {
        setLog(result.data.Data);
        renderAuditTrail(result.data.Data);
      }
    ...
    window.$("#audit-trail").DataTable({
      ...
      columns: [
        ...
        },
        {
          ...
          render: (data, type, row, meta) => {
            if (type === "display") {
              return row.Date ? moment(row.Date).format("yyyy/MM/DD hh:mm") : "" ;
            }
            return row.Date ? moment(row.Date).format("yyyy/MM/DD hh:mm") : "" ;
          },
              ...
              <div className="row h-100">
                ...
                >
                  <div className="px-1 py-2">
                    <Header className="mb-2">Filter Audit Trail</Header>
...
export default connect(null, actions)(FilterAuditTrail);
